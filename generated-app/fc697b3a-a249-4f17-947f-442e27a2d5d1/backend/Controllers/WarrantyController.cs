...
public class WarrantyController : ControllerBase
{
    ...
    {
        var total = await _context.Warranties.CountAsync();
        var active = await _context.Warranties.CountAsync(w => w.Status == "Active");
        var expiring = await _context.Warranties.CountAsync(w => w.Status == "Expiring Soon");
        var totalPremium = await _context.Warranties.SumAsync(w => w.Premium);
        
        return new { total, active, expiring, totalPremium };
    }

    [HttpPost]
    public async Task<ActionResult<Warranty>> CreateWarranty(Warranty warranty)
    {
        warranty.PolicyNumber = $"WRN-{DateTime.Now:yyyy}-{(_context.Warranties.Count() + 1):D3}";
